...
public class GUI implements ActionListener
{
	...
	GUI ()
	{
		...
		frame = new JFrame("NitroGuy's ColorSplitter");
	...
	public void actionPerformed (ActionEvent e)
	{
		...
		else if (e.getActionCommand().equals("choose input folder"))
		{
			...
			if (ColorSplitter.inputFolder != null)
			{
				fileChooser.setCurrentDirectory(ColorSplitter.inputFolder);
			}
			fileChooser.setFileSelectionMode(JFileChooser.DIRECTORIES_ONLY);
			int option = fileChooser.showOpenDialog(frame);
			if (option == JFileChooser.APPROVE_OPTION)
			{
				ColorSplitter.inputFolder = fileChooser.getSelectedFile();
				inputLabel.setText(ColorSplitter.inputFolder.getAbsolutePath());
			}
		}
		else if (e.getActionCommand().equals("choose output folder"))
		{
			JFileChooser fileChooser = new JFileChooser();
			if (ColorSplitter.outputFolder != null)
			{
				fileChooser.setCurrentDirectory(ColorSplitter.outputFolder);
			}
			fileChooser.setFileSelectionMode(JFileChooser.DIRECTORIES_ONLY);
			int option = fileChooser.showSaveDialog(frame);
			if (option == JFileChooser.APPROVE_OPTION)
			{
				ColorSplitter.outputFolder = fileChooser.getSelectedFile();
				outputLabel.setText(ColorSplitter.outputFolder.getAbsolutePath());
			}
		}
		else if (e.getActionCommand().equals("new split"))
		{
			centerPanel.remove(newSplitButton);
			SplitSetting newSplit = new SplitSetting(this);
			ColorSplitter.splits.add(newSplit);
			centerPanel.add(newSplit);
			if (ColorSplitter.splits.size() < MAX_SPLITS)
			{
				centerPanel.add(newSplitButton);
			}
		...
		else if (e.getActionCommand().equals("save"))
		{
			...
			{
				String saveString = "";
				for (SplitSetting setting : ColorSplitter.splits)
				{
	...
	public void removeSplitSetting (SplitSetting ss)
	{
		...
		ColorSplitter.splits.remove(ColorSplitter.splits.indexOf(ss));
		...
		try
		{
			for (String setting : config.split("\n"))
			{
				if (!setting.isEmpty())
				{
					...
					SplitSetting newSplit = new SplitSetting(reader.nextLine().substring(1), newColor, this);
					ColorSplitter.splits.add(newSplit);
					centerPanel.add(newSplit);
	...
	{
		JOptionPane.showMessageDialog(frame, "Error reading image file: " + fileName, "Darn", JOptionPane.ERROR_MESSAGE);
	}
	
	public void errorWriting ()
	{
		JOptionPane.showMessageDialog(frame, "Error writing image file", "Darn", JOptionPane.ERROR_MESSAGE);
	}
}